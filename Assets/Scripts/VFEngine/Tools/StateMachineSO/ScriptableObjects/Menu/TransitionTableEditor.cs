...
namespace VFEngine.Tools.StateMachineSO.ScriptableObjects.Menu
{
    ...
    internal class TransitionTableEditor : Editor
    {
        ...
        {
            if (!_displayStateEditor) TransitionTableGUI();
            else StateEditorGUI();
        }
        ...
        private void StateEditorGUI()
        {
            ...
            // Back button
            if (GUILayout.Button(EditorGUIUtility.IconContent("scrollleft"), GUILayout.Width(35),
                GUILayout.Height(20)) || _cachedStateEditor.serializedObject == null)
            {
            ...
            for (var i = 0; i < _fromStates.Count; i++)
            {
                ...
                var headerRect = BeginHorizontal();
                {
                    ...
                    // Toggle
                    {
                        ...
                        _toggledIndex = EditorGUI.BeginFoldoutHeaderGroup(toggleRect, _toggledIndex == i, label,
        ...
        {
            if (_cachedStateEditor == null) _cachedStateEditor = CreateEditor(state, typeof(StateEditor));
            else CreateCachedEditor(state, typeof(StateEditor), ref _cachedStateEditor);
        ...
        internal void ReorderState(int index, bool up)
        {
            ...
            if (toggledState) _toggledIndex = _fromStates.IndexOf(toggledState);
        }
        ...
        private bool TryGetExistingTransition(SerializedProperty from, SerializedProperty to, out int fromIndex,
            out int toIndex)
        {
            ...
            toIndex = _transitionsByFromStates[fromIndex].FindIndex(transitionHelper =>
                transitionHelper.SerializedTransition.ToState.objectReferenceValue == to.objectReferenceValue);
            return toIndex >= 0;
        }
        ...
        private void GroupByFromState()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                {
                    Debug.LogError("Transition with invalid \"From State\" found in table " +
